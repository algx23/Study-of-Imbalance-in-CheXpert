...
class FocalLoss():
    ...
    def __init__(self, alpha: Tensor, gamma: float)  -> None:
        self.alpha = alpha
        self.gamma = gamma
        self.bce_loss = BCEWithLogitsLoss() # needed to calculate the log(p) part of the equation
        return

    def calculate_focal_loss(self, logits: Tensor, true_labels: Tensor) -> Tensor:
        """
        ...
        ce = self.bce_loss(logits, true_labels, reduction="none")
        # compute the loss across all classes and then return it as a single value
        # so that it fits with my existing training loop
        focal_loss = ce*(self.alpha*true_labels*(1-p)**self.gamma + (1-self.alpha)*(1-true_labels)*p**self.gamma)
